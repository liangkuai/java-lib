...
Java 是一门面向对象的编程语言，在 Java 程序运行过程中无时无刻都有对象被创建出来。在语言层面上，创建对象（包括克隆、反序列化）通常仅仅是一个 new 关键字而已，而在虚拟机中，对象的创建又是怎么样一个过程呢？
...
创建对象是为了使用对象，Java 程序会通过虚拟机栈上的 reference 来操作堆中的实际对象。

但由于 reference 类型在 JVM 规范中只规定了它是一个指向对象的引用，并没有定义这个引用应该通过何种方式去定位、访问堆中对象的具体位置，所以对象访问方式也是取决于虚拟机如何实现。
...
> HotSpot VM 使用第二种直接指针的方式进行对象访问的。